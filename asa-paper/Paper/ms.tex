...
In political methodology terms, the problem is therefore \emph{measurement design}: how do we scale retrieval-augmented coding of elite attributes while preserving (i) auditability (a third party can verify why a label was produced), (ii) cross-national comparability (labels remain in a stable, closed set of classes), and (iii) temporal validity (evidence respects an ``as-of'' target).
