...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={rounded corners, draw=black, thick, fill=gray!3, inner sep=6pt, align=left},
    pill/.style={rounded corners=9pt, draw=black, thick, fill=white, inner sep=5pt, align=center},
    arr/.style={-{Stealth[length=3mm]}, thick},
  ]
    \node[box, minimum width=0.30\linewidth] (input) {
    ...
    \node[box, right=of input, minimum width=0.34\linewidth] (agent) {
    ...
    \node[box, right=of agent, minimum width=0.30\linewidth] (output) {
    ...
    \node[pill, below=10mm of agent, minimum width=0.78\linewidth] (why) {
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[
    ...
    comp/.style={rounded corners, draw=black, thick, fill=gray!3, inner sep=6pt, align=left},
    ...
    \node[comp, minimum width=0.36\linewidth] (spec) {
      \textbf{Task spec}\\[-1pt]
      \(\langle\)person, country, year\(\rangle\)\\
      codebook \(\mathcal{C}\)\\
    ...
    \node[comp, right=of spec, minimum width=0.40\linewidth] (graph) {
    ...
    \node[tool, above right=of graph, xshift=6mm] (wiki) {Wikipedia\\retriever};
    \node[tool, below right=of graph, xshift=6mm] (web) {Web search\\(DDG)};
    \node[tool, right=of graph, xshift=26mm] (time) {Temporal\\verifier};

    \node[comp, below=of graph, minimum width=0.40\linewidth] (post) {
    ...
    \node[store, right=20mm of post] (db) {Trace store\\(\texttt{SQLite})};

    \node[comp, left=of post, minimum width=0.36\linewidth] (human) {
    ...
    \draw[loop] (human.north) |- (spec.west);
  \end{tikzpicture}
