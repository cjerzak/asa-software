...
%% TEMP
\input{./FiguresParties/InitialInputStatsParties.tex}
...
In this study, we offer a new approach to the measurement of ethnic cleavages. This approach has several distinctive characteristics that, collectively, set it apart from extant work. It is centered on elites rather than masses; it treats cleavages as matters of degree rather than of kind; it encompasses individual parties and party systems as well as individual ethnic groups and ethnic systems; and it is applicable to any setting where the ethnic identity of elites can be ascertained. 

The key indicator is the ethnic composition of parliamentary party delegations. We describe delegations as \textit{integrated} if there are few ethnic differences and \textit{segregated} if each party represents a different ethnic group. 

To operationalize this approach we offer two measures that capture different dimensions of integration/segregation. The first index, \textit{homogeneity}, measures how homogeneous the content of each unit is, e.g., how ethnically pure each political party is --- but not how distinctive they are from each other.


employ an algorithm derived from optimal transport theory \citep{lott2009ricci}, which generates an index bounded at zero (perfect integration) and one (perfect segregation). Since values increase with greater segregation, we refer to the resulting scale as a \textit{segregation index}.
...
\input{./FiguresParties/PartyGroupExamples.tex}
...
To provide a nuanced metric to express this continuum we propose to focus on the magnitude of the transformations required for (a) a party or (b) a legislature to reach perfect integration. The proposed scale varies from 0 (where no changes are required) to a value that approaches 1 asymptotically (where nearly everything needs to change). Since values increase as one moves further away from perfect integration, we refer to this as a \textit{segregation index}.
...
Consider the example illustrated in Table \ref{tab:ThreePartyTable}. Here, we see a legislature with sixteen MPs, three parties, and two ethnic groups. The majority group, \textit{A}, comprises 3/4 of the legislatures while the minority group, \textit{B}, comprises 1/4.
...
Let us now consider how to operationalize these intuitions in a precise fashion across situations where there is variability in the number and size of political parties and ethnic groups, and in the size of legislatures. 
...
For the purposes of our analysis, we impose an infinite cost on changing ethnicities and normalize the costs so that the resulting measure falls within the interval $[0, 1]$. This normalization allows for meaningful comparisons across different political systems and time periods. 
...
We then compare this measure with other measures of ethnic parties. I have asked Kanchan Chandra and Ishiyama for their data on ethnic parties. No word back from either, so far. Not sure if the former is public or if it was ever completed. In any case, we ought to be able to obtain measures of ethnic parties from \citet[App II]{lublin2014minority} and of ethnic voting from \citet{houle2019structure}.

% houle2019structure
The ethnic voting measure from \citet{houle2019structure} is defined as: 
\begin{align*}
...
\textit{H1: Party system}. Larger party systems (measured by the effective number of parliamentary parties) are conducive to ethnic parties. Unit of analysis: countries.

\textit{H2: Parties}. Smaller parties (measured by number of seats) are more conducive to ethnic parties. Unit of analysis: parties.
...
\textit{H5: Electoral rules}. SMDs favor ethnic parties. (Assumption 1: most ethnic groups are spatially segregated. Assumption 2: ethnic groups perceive each other as rivals, so a party that appeals to one group alienates other groups.) Unit of analysis: countries or tiers in mixed electoral systems.
...
\subsection{Representing Party-Ethnic Structures as Bipartite Graphs}

A useful way to conceptualize the relationship between parties and ethnic groups, and to assess patterns of ethnic cleavage, is to model the legislature as a bipartite graph. One set of nodes $\mathcal{P} = \{p_1, p_2, \dots, p_M\}$ represents the set of political parties, and the other set $\mathcal{G} = \{g_1, g_2, \dots, g_N\}$ represents the set of ethnic groups observed in the legislature. We place an edge between $p_j$ and $g_i$ if and only if party $p_j$ includes at least one legislator from ethnic group $g_i$. This graph-based representation provides a direct, visual, and mathematical framework for understanding the structure of ethnic cleavages at the elite level. Formally, we can represent this structure through an incidence matrix $\mathbf{O} = [o_{ij}]$, where $o_{ij}$ denotes the count of legislators in party $p_j$ belonging to ethnic group $g_i$. In graph-theoretic terms, $\mathbf{O}$ is the biadjacency matrix of the bipartite network. 

While our segregation index, defined through an optimal transport framework, captures the degree of ethnic-party alignment, the bipartite graph model provides additional insights by allowing us to identify cohesive communities of ethnic groups that commonly occur across parties. By projecting the bipartite graph onto the ethnic-group set, we obtain a one-mode network $\mathbf{A} = [a_{ij}]$ in which each node corresponds to an ethnic group and edges are weighted by their co-occurrence in the same parties:
\[
a_{ij} = \sum_{k=1}^{M} \mathbbm{1}(o_{ik} > 0 \wedge o_{jk} > 0),
\]
or, more generally, $a_{ij}$ can be defined proportionally to the number of legislators shared across parties. In essence, $\mathbf{A}$ encodes how closely aligned or segmented ethnic groups are, indirectly reflecting the ethnic cleavage structure discussed above.

Applying a greedy community detection algorithm \citep{clauset2004finding} to this projected network can reveal clusters of ethnic groups that co-occur more frequently than would be expected at random, effectively mapping ethnic “neighborhoods” within the legislature. Within these clusters, ethnic groups may share common political platforms, overlapping policy preferences, or historical ties. Identifying these communities complements our segregation index by uncovering latent structures that would remain hidden in a simple two-dimensional tabulation of parties and groups.
...
% scratch
% we could distinguish between exogenous elements: ethnicity (the number and size of ethnic groups in the population)
    the size of the legislature

% and endogenous elements:
%the number of parties that gains representation in the legislature the size of these parties (in seats)
    the distribution of ethnicities across the parties
