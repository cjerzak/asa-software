...
The **asa** package provides an LLM-powered research agent for performing AI search tasks at large scales. It defaults to the OpenCode CLI runtime with ASA-managed provider routing and search tools, and also includes a built-in LangGraph runtime. It implements a ReAct (Reasoning + Acting) agent pattern that combines:
...
- **Multiple LLM Backends**: Support for OpenAI, Azure OpenAI, Groq, xAI, Gemini, Anthropic, Bedrock, OpenRouter, and Exo
- **Multiple Agent Runtimes**: OpenCode by default, plus the built-in LangGraph runtime and free-code
...
The package uses Python for ASA-managed search tools, provider gateways, and the built-in LangGraph runtime. Run this once to set up the environment:
...
```
This installs LangChain, LangGraph, search tools, gateway helpers, and proxy support packages. The default OpenCode runtime also requires the `opencode` CLI on `PATH` or `ASA_OPENCODE_BIN`.
...
# Also supported:
# Sys.setenv(AZURE_OPENAI_ENDPOINT = "https://<resource>.openai.azure.com")
# Sys.setenv(AZURE_OPENAI_API_KEY = "your-azure-openai-api-key")
# Sys.setenv(AZURE_OPENAI_DEPLOYMENT = "your-azure-deployment")
# Sys.setenv(GOOGLE_API_KEY = "your-gemini-api-key")      # or GEMINI_API_KEY
# Sys.setenv(ANTHROPIC_API_KEY = "your-anthropic-api-key")
# Sys.setenv(OPENROUTER_API_KEY = "your-openrouter-api-key")
# Sys.setenv(AWS_ACCESS_KEY_ID = "...")
# Sys.setenv(AWS_SECRET_ACCESS_KEY = "...")
# Exo uses a local server and does not require an API key.
```
...
asa supports `openai`, `azure-openai`, `groq`, `xai`, `gemini`, `anthropic`, `bedrock`, `openrouter`, and `exo`. The examples below show a few common setups:
...
agent_openrouter <- initialize_agent(
  backend = "openrouter",
...
agent <- initialize_agent(
  ...
  memory_keep_recent = 2     # Keep 2 most recent exchanges intact
)
...
Each agent response includes a text trace in `raw_output` and, when available,
a structured JSON trace in `trace_json`:
...
# Prefer the structured trace when available
trace <- if (!is.null(result$trace_json) && nzchar(result$trace_json)) {
  result$trace_json
} else {
  result$raw_output
}
cat(substr(trace, 1, 500))  # First 500 characters
```
...
# Extract structured data from the preferred trace representation
trace <- if (!is.null(result$trace_json) && nzchar(result$trace_json)) {
  result$trace_json
} else {
  result$raw_output
...
```{r process-outputs}
# After running a batch on a data frame
results_df <- run_task_batch(
  data.frame(prompt = prompts),
  agent = agent
)

# raw_output is included automatically and can be processed directly
